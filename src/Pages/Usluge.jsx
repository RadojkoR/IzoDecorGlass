...
function Usluge() {
  ...
  const usluge = [
    ...
    { uslugeHeading: "Tuš Kabine", para: "Tuš kabine predstavljaju savršeno rešenje za funkcionalnost i estetiku vašeg kupatila. Bilo da želite moderan dizajn ili klasičan stil, naše tuš kabine pružaju udobnost, praktičnost i lako održavanje.", img: tusKabina, paralaxClass: "paralaxTusKabine", showBtnClass: "d-block", url: "/usluge/tus-kabine" },
    { uslugeHeading: "Staklene Ograde", para: "Staklene ograde donose savršen spoj sigurnosti i sofisticiranog dizajna. Idealan su izbor za unutrašnje i spoljašnje prostore, pružajući otvoren i moderan izgled uz vrhunsku izdržljivost i lako održavanje.", img: staklenaOgrada, paralaxClass: "paralaxStakleneOgrade", showBtnClass: "d-block", url: "/usluge/staklene-ograde"  },
    ...
    <>
      ...
      {usluge.length > 0 ? (
        usluge.map((item, index) => (
          <UslugeItem key={index} heading={item.uslugeHeading} para={item.para} img={item.img} paralaxClass={item.paralaxClass} showBtn={item.showBtnClass} url={item.url}/>
        ))
